...
func TestReElection2A(t *testing.T) {
	...
	log.Println("### 0")
	cfg.connect(leader1)
	// Q: Should expect to have only one leader here? With the disconnected-rejoined one change to follower?
	leader2 := cfg.checkOneLeader()
	log.Println("### 1")
	log.Printf("Leader1 :%d\tLeader2: %d\n", leader1, leader2)
	// if there's no quorum, no leader should
	// be elected.
	// Q? In here, the previous leader timeout?
	time.Sleep(2 * RaftElectionTimeout)
